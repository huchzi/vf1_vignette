...
The \texttt{glr()} function estimates a regression model for global indices. For the example series, the patient loses \texttt{getgl(example3)} dB/year, with $p < \texttt{getgl(example3)}$.

%\texttt{r round(abs(glr(getgl(example3))$sl), 2)}
%\texttt{r format(ceiling(1000 * glr(getgl(example3))$pval) / 1000, nsmall = 3)}
