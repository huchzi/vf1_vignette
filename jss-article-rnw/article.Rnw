...
\begin{table}[h]
\centering
\caption{Global indices used in Humphrey perimetry. Note that calculation may differ slightly for other perimeters.}
\label{tab:globalIndices}
\begin{tabular}{|p{1.5cm}|p{3.5cm}|p{8cm}|}
\hline
\textbf{Variable} & \textbf{Meaning} & \textbf{Calculation} \\
\hline
\textbf{msens} & Mean sensitivity & \[\mathrm{MS} = \frac{\sum_{i=1}^{n} w_i \cdot S_i}{\sum_{i=1}^{n} w_i}\] \\
\hline
\textbf{ssens} & SD of sensitivity & \[\text{SS} = \sqrt{\frac{1}{n} \sum_{i=1}^{n} (S_i - \overline{S})^2}\] \\
\hline
\textbf{tmd} & Mean total deviation & \[\mathrm{MD} = \frac{\sum_{i=1}^{n} w_i \cdot TD_i}{\sum_{i=1}^{n} w_i}\] \\
\hline
\textbf{tsd} & SD of total deviation & \[\text{TSD} = \sqrt{\frac{1}{n} \sum_{i=1}^{n} (TD_i - \overline{TD})^2}\] \\
\hline
\textbf{pmd} & Mean pattern deviation & \[\mathrm{PMDS} = \frac{\sum_{i=1}^{n} w_i \cdot (TD_i - GH)}{\sum_{i=1}^{n} w_i}\] \\
\hline
\textbf{psd} & SD of pattern deviation & \[\text{PSD} = \sqrt{\frac{1}{n} \sum_{i=1}^{n} (PD_i - \overline{PD})^2}\] \\
\hline
\textbf{gh} & General height & \[GH = \mathrm{Median}\left( \left\{ TD_i \mid i \in \text{Top } k \right\} \right)\] \\
\hline
