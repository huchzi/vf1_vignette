...
\author{Cord Huchzermeyer\\INSTITUTION
   \And Iván Marín-Franch\\Computational Optometry}
\Plainauthor{Cord Huchzermeyer, Iván Marín-Franch}

\title{visualFields 1.x: Tools for Analyzing the Field of Vision in \proglang{R}}
\Plaintitle{visualFields 1.x: Tools for Analyzing the Field of Vision in R}
\Shorttitle{visualFields 1.x in \proglang{R}}
...
\subsection{Getting started with the package}
...
\subsection{Identifying defective locations on the visual field}
...
\subsection{Normative dataset and normative references}
...
\subsubsection{Age linear models and mean normal visual fields}
...
\subsection{Total and Pattern deviation probabilities}
...
\begin{leftbar}
This section can include details about the methods implemented in the \pkg{visualFields} package, such as statistical models, algorithms, or visualization techniques. For now, this section is left as a placeholder.
\end{leftbar}
...
\begin{leftbar}
This section can include results obtained using the \pkg{visualFields} package, such as example analyses or visualizations. For now, this section is left as a placeholder.
\end{leftbar}
