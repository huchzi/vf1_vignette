...
\usepackage{orcidlink,thumbpdf,lmodern,amsmath}
...
\Address{
  ...
  E-mail: \email{cord.huchzermeyer@uk-erlangen.de}\\
  \\
  Giovanni Montesano\\
...
In healthy vision, most visual functions, including light sensitivity, decline with increasing eccentricity—i.e., distance from the center of fixation. This decline in differential light sensitivity gives rise to a three-dimensional surface known as the hill of vision, also referred to as Traquair’s island of vision \citep{IMF:Traquair1938}. Figure~\ref{fig:hill} shows a stimulus presentation in a half-bowl perimeter and a three-dimensional representation of the hill of vision.
...
Sensitivity is the inverse of threshold, and in perimetry, it is generally reported relative to the maximum intensity threshold that is tested. This maximum intensity threshold $\Delta L_{\rm max}$ can differ among perimeters. For the Humphrey Field Analyzer (HFA) the maximum luminance increment is $L_{\rm max} = 10{,}000\,asb$. You cannot directly compare sensitivity values between perimeters that use different $L_{\rm max}$. Sensitivity is expressed on a logarithmic scale, the decibel (dB), thus,
...
\begin{itemize}
  \item SUNY-IU dataset (\code{vfctrSunyiu24d2}) for 24-2 with Goldmann size III \citep{IMF:Mar2013b} and SITA standard algorithm
  \item Iowa PC26 (\code{vfctrIowaPC26}) and Peri (\code{vfctrIowaPeri}) datasets for Goldmann size V \citep{IMF:Marin2018c} and the ZEST algorithm
\end{itemize}

The normal values derived from these data can be found in \code{normvals}. Normative datasets and references are very critically dependent on the test (SITA, ZEST) so data from other perimeters or other Bayesian thresholding algorithms should not be analyzed with these references values.
...
To calculate the total deviation, use the function \code{gettd()} from the normative values, do:
...
Or automatically with \code{getpd()}:
...
Even healthy eyes may show unusually small deviation values. To interpret these values, we calculate the probability that a deviation reflects pathology using empirical distributions from the normative data \citep{IMF:Hei1987a, IMF:Heijl1991}. These are based on the look up tables for probabilities:
...
\caption{Global indices used in Humphrey perimetry. Note that calculations and indices differ for other perimeters.}
\label{tab:globalIndices}
\begin{tabular}{|p{1.5cm}|p{6.0cm}|p{6.5cm}|}
\hline
\textbf{Variable} & \textbf{Meaning} & \textbf{Calculation} \\
\hline
\textbf{msens} & Mean Sensitivity & \[\frac{1}{n} \sum_{i=1}^{n} s_i\] \\
\hline
\textbf{ssens} & SD of Sensitivity & \[\sqrt{\frac{1}{n} \sum_{i=1}^{n} (s_i - \overline{s})^2}\] \\
\hline
\textbf{tmd} & Mean Deviation (MD) & \[\sum_{i=1}^{n} w_i t_i\] \\
\hline
\textbf{tsd} & SD of Total Deviation & \[\sqrt{\sum_{i=1}^{n} w_i (t_i - \overline{t})^2}\] \\
\hline
\textbf{pmd} & Mean Pattern Deviation (MPD) & \[\sum_{i=1}^{n} w_i p_i\] \\
\hline
\textbf{psd} & Pattern Standard Deviation (PSD) & \[\sqrt{\sum_{i=1}^{n} w_i (p_i - \overline{p})^2}\] \\
\hline
\textbf{gh} & General Height (GH) & $85^{\rm th}$ TD percentile\tnote{a}\\
\hline
\textbf{vfi} & Visual Field Index (VFI) & see \citep{IMF:Ben2008} \\
\hline
\end{tabular}
\begin{tablenotes}[flushleft]
\item$\boldsymbol n$: Number of locations, 52 for the 24--2.
\item$\boldsymbol t_i$: Pointwise TD value at location $i = 1, \cdots, n$. Their average is denoted by $\overline{t}$.
\item$\boldsymbol p_i$: Pointwise PD value at location $i = 1, \cdots, n$. Their average is denoted by $\overline{p}$.
\item$\boldsymbol w_i$: Pointwise weights for MD, MPD, standard deviation of TD, and PSD $i = 1, \cdots, n$. The weights are the inverse of the pointwise standard deviations of, respectively, TD or PD normalized to add up to 1.
\item[a] For 24--2, it is the $7^{\rm th}$ greatest TD value.
\end{tablenotes}
\end{threeparttable}
\end{table}

Global indices summarize the visual field by applying summary statistics on sensitivity, total deviation or pattern deviation. These indices are shown in Table \ref{tab:globalIndices} and calculated using \code{getgl()}. The resulting \code{data.frame} also contains the patient's age, eye, and date of the test.
