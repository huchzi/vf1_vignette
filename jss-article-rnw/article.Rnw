...
For characterizing local variation of the visual field, the HFA uses pattern standard deviaton (PSD), which is the weighted standard deviation of the TD values minus the general height.

The visual field index (VFI) is a global parameter that, similar to the MD, summarizes the overall loss of sensitivity, but should be less strongly affected by cataract \citep{IMF:Ben2008}. Catarct (opacification of the crystalline lens) is an important confounder in longitudinal follow-up of glaucoma patients, because surgical treatments for glaucoma may trigger cataract and cataract leads to deterioration of the visual field in the absence of glaucoma progression. It is expressed in percent, where a VFI of 100\%corresponds to a normal field and 0\% corresponds to blindness.
