...
\hline
\textbf{Static Perimetry} & A method of visual field testing in which stimuli are presented at predefined locations with varying intensities to determine the minimum visible stimulus at each point. \\
\hline
...
\hline
\textbf{Threshold} & The minimum stimulus intensity that a patient can detect at a specific location in the visual field with a certain probability (usually 50\%). \\
\hline
\textbf{Sensitivity} & The inverse of threshold on a logarithmic scale (dB); higher sensitivity means that lower intensity stimuli can be detected at a given location. \\
\hline
\textbf{Location Grid} & A predefined array of points across the visual field at which stimuli are presented to assess visual function. \\
\hline
...
Static automated perimetry (SAP) aims to estimate the hill of vision as accurately as possible within the constraints of clinical practice. It is performed inside a dome-shaped device (a Ganzfeld perimeter) to ensure a uniform distance between the eye and all stimulus locations. In contrast, campimetry is performed on a flat surface. With the advent of new technology, new visual field tests basesd on campymetry are being developed on flat screens on computers, tables, phones and, more notably, virtual-reality systems. They are becoming more common in particular for use in telemedicine.
