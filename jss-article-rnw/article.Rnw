...
\usepackage{orcidlink,thumbpdf,lmodern,amsmath,listings}
...
\author{
  Cord Huchzermeyer~\orcidlink{0000-0002-0408-8981}\\
  Dpt. of Ophthalmology\\
  University Hospital Erlangen
  \And
  Giovanni Montesano~\orcidlink{0000-0002-9148-2804}\\
  Moorfields Eye Hospital
  \And
  Iván Marín-Franch~\orcidlink{0000-0003-0552-8456}\\
  Computational Optometry
}
\Plainauthor{Cord Huchzermeyer, Giovanni Montesano, Iván Marín-Franch}


\title{visualFields: An \proglang{R} package for Analyzing Visual Field Data}
\Plaintitle{visualFields: An R package for Analyzing Visual Field Data}
\Shorttitle{visualFields in \proglang{R}}

\Abstract{
  Visual field testing (perimetry) is an important tool for measuring visual function across the field of vision using dedicated perimeters. The \pkg{visualFields} package is part of the Open Perimetry Initiative (OPI), 
  a standard for interfacing with such visual field testing machines. This article 
  gives an brief review of the principles underlying perimetry and introduces the package, which provides tools for analyzing the field of vision, including visualization, statistical analysis, and clinical interpretation of visual-field loss and its progression over time.
}
...
\Address{
  ...
  City St George's, University of London, Optometry and Visual Sciences, London, United Kingdom \\
  E-mail: \email{giovmontesano@gmail.com}\\
  \\
  Iván Marín-Franch\\
...
The visual field refers to the entire area that can be seen at a given moment, encompassing both central and peripheral vision. Locations in the visual field are typically measured in degrees of visual angle, with the fixation point defined as the origin at $x = 0^\circ$ and $y = 0^\circ$. Locations temporal and superior to fixation are assigned positive angles, whereas nasal and inferior locations are assigned negative angles.

In healthy vision, most visual functions, particularly light sensitivity, decline with increasing eccentricity—i.e., distance from the center of fixation. This decline in differential light sensitivity gives rise to a three-dimensional surface known as the hill of vision, also referred to as Traquair’s island of vision \citep{IMF:Traquair1938}. Figure~\ref{fig:hill} shows a stimulus presentation in a half-bowl perimeter and a three-dimensional representation of the hill of vision.
...
\emph{Perimetry} (visual field testing) quantifies the visual field by measuring a patient's sensitivity to light increments on a steady background at various locations within the field. There are different types of perimetry. Static automated perimetry (SAP) measures sensitivities at discrete locations in the field (in contrast to kinetic perimetry, which uses moving stimuli in order to determine lines of identical sensitivity, called isopters) and aims to estimate the hill of vision as accurately as possible within the constraints of clinical practice. It is performed inside a dome-shaped device (a Ganzfeld perimeter) to ensure a uniform distance between the eye and all stimulus locations. Campimetry is performed on a flat surface. With the advent of new technology, new campimetric visual field tests are being developed on flat screens on computers, tables, phones and, most notably, virtual-reality systems. Important terms used for perimetry are shown in Table 1.

The \pkg{visualFields} package is only concerned wit \emph{static perimetry}. In static perimetry, a stimulus is presented at discrete locations and varied according to the observer's responses in order to determine the threshold, i.e. the light increment where the stimulus is barely perceptible. Once a stimulus is presented, the observer has a set time window to press a button, signalling that the stimulus was perceived. A lack of response is taken to mean that the stimulus was not perceived, resulting in a simple yes/no outcome. Stimuli are presented several times for each location, but the presentations with different intensities are randomly interleaved between the different stimulus location, so that the observer does not know at which position in the visual field to expect the next presentation. Perimetry has scientific and clinical applications.

\begin{table}[ht]
\centering
\caption{Key Terms in Perimetry}
\begin{tabular}{|p{4cm}|p{10cm}|}
\hline
\textbf{Term} & \textbf{Definition} \\
\hline
\textbf{Static Perimetry} & A method of visual field testing in which stimuli are presented at predefined locations with constant size and varying intensities to determine the minimum visible stimulus at each point. \\
\hline
\textbf{Kinetic Perimetry} & A technique where a stimulus is moved from non-seeing to seeing areas to map the borders of the visual field. Cannot be analyzed with \pkg{visualFields.} \\
\hline
\textbf{Campimetry} & Visual field testing on a flat surface instead of a hemispherical bowl. \\
\hline
\textbf{Threshold} & The minimum stimulus intensity that a patient can detect at a specific location in the visual field (barely perceptible stimulus). Because perception close to threshold has a random component, a criterion probability (usually 50\%) is used. \\
\hline
\textbf{Sensitivity} & Also threshold sensitivity. The inverse of threshold on a logarithmic scale (dB); higher sensitivity means that lower intensity stimuli can be detected at a given location. \\
\hline
\textbf{Location Grid} & Spatial arrangement of the test locations. A predefined array of points across the visual field at which stimuli are presented to assess visual function. \\
\hline
\textbf{Hill of Vision} & A three-dimensional representation of visual sensitivity across the visual field, with peak sensitivity at fixation and decreasing sensitivity toward the periphery. \\
\hline
\end{tabular}
\end{table}

\subsection{How do we measure sensitivity in perimetry?}

Perimetry is a psychphysical test. Psychophysical tests measure the relationship between physical stimulus strength (i.e. intensity of light or more precisely stimulus contrast for a constant background) and an observer's perception. This is achieved by systematically varying the stimulus strength across discrete trials based on the observer's responses. 
...
The most straightforward method for estimating this threshold is the method of constant stimuli, in which stimuli are presented multiple times at several discrete, fixed intensities around the anticipated threshold. This approach yields an estimate of the psychometric function, which is typically modeled using a cumulative distribution function (CDF), most commonly the Gaussian or Weibull CDF. However, obtaining precise estimates of the psychometric function’s parameters — such as the intensity threshold, the slope at threshold, and the false positive and false negative rates — requires a large number of trials. Long test duration negatively affect attention and make any test impractical for clinical use.
In clinical settings, the observer is restricted to a limited set of trials, and the stimuli are varied according to a predefined algorithm

Two classic alternative approaches to estimate the 50\% threshold without measuring the full psycometric function are the \emph{staircase} and \emph{the full threshold} (the latter consisting of two staircase algorithms ran one after the other). Staircase algorithms start by presenting a stimulus at an initial guess. If the stimulus is seen, the intensity is decreased at the next presentation in steps of a predetermined value. If the current stimulus is not seen, the intensity of the following presentation is increased instead. After the first reversal (transition from seen to not seen), the stimulus intensity is increased in dB steps equal to half the original step size (e.g. 4 dB in the first phase, 2 dB in the second phase). At the second reversal (not seen to seen) the algorithm stops. The estimated threshold is typically obtained by averaging the intensities of the last two presentations. Staircase and its modified, full threshold version, which was the standard when static automated perimetry was first introduced, see e.g., \citep{IMF:Heijl1985}, is generally too long and impractical for clinical use.

For this reason, psychophysical Bayesian algorithms \citep{IMF:Taylor1967,IMF:Kin1994} where introduced to reduce the number of stimulus presentations, balancing precision and test duration. Some algorithms, such as the \emph{Swedish Interactive Threshold Algorithm} (SITA) used in the Humphrey Field Analyzer, also incorporate information from adjacent locations in the visual field.

Fixed-threshold estimates are imported from a perimeter into the \pkg{visualFields} package. As individual stimulus–response data are not included, modeling the full psychometric function is not possible. The underlying thresholding algorithm remains relevant, however, as data collected using different algorithms should not be mixed.

\subsection{How is sensitivity quantified?}

The threshold is a measure of the increment in light intensity that is barely perceptible. Thus it is a measure of difference in luminance. In perimetry, luminance is usually reported in apostilbs rather than ${\rm cd/m^2}$, where $\pi \times {\rm apostilbs} = 1 {\rm cd/m^2}$. The background is nominally $10 {\rm cd/m^2}$ or 31.4 apostilbs for standard photopic perimetry, and the intensity threshold, $\Delta L_{\rm thr}$, is the stimulus luminance at threshold minus $10 {\rm cd/m^2}$. 

Sensitivity is the inverse of threshold, and in perimetry, it is always reported relative to the maximum intensity threshold that is tested. This maximum intensity threshold $\Delta L_{\rm max}$ can differ among perimeters. For the Humphrey Field Analyzer (HFA) the maximum luminance increment is $L_{\rm max} = 10{,}000\,asb$. You cannot directly compare sensitivity values between perimeters that use different $L_{\rm max}$. Sensitivity is expressed on a logarithmic scale, the decibel (dB), thus,

\begin{equation}
dB = 10\ \rm log \left (\frac{\Delta L_{\rm max}}{\Delta L_{\rm thr}} \right)\ .
\end{equation}

Since the maximum luminance differential is $\Delta L_{\rm max}$ the fraction is $\ge 1$ (i.e. dB $\ge 0$) with 0 being the maximum stimulus possible, and 0\,dB therefore is the minimum sensitivity that can be measured by the perimeter. Decibel (dB) scales are normally limited to 40 dB which corresponds to 1 asb for the HFA.

\subsection{Aims of the visualFields package}

The \pkg{visualFields} package is an aid for the standardization of visual field data analysys from any device running SAP. It provides the following functionality:
\begin{itemize}
  ...
  \item A standardized format for the visualization of plotting the visual fields sensitivity, TD, and PD maps {\code(data(gpars))}
...
In Section \ref{sec:methods}, we will show how the data supplied by the package can be analyzed with the default environment settings. In Section \ref{sec:results}, we will show how to plot the data, perform longitudinal and cross-sectional analyses, how to import data from the Humphrey perimeter and how to set up the environment for these data. Section \ref{sec:others} outlines support for non-conventional settings and other perimeters. Final remarks are shown in Section \ref{sec:summary}.
...
Below is a list of thet datasets included in \pkg{visualFields}:
<<eval=FALSE>>=
...
vfpwgSunyiu24d2   # Series of HFA 24-2 SITA standard tests for the two eyes
                  # of a patient followed up over time
vfpwgRetest24d2   # 30 patients tested each 12 times with the HFA 24-2 SITA standard
@
...
vfisvalid(example1)
@
Here, the visual field data is not valid, because an illegal column was added.
...
\begin{table}[h]
\centering
\caption{Visual field patterns included in the visualFields package.}
\label{tab:locmaps}
...
locmap_table <-
  data.frame(Name = sapply(locmaps, function(lm) { return(lm$name) }),
             Description = sapply(locmaps, function(lm) { return(lm$desc)}))

xtable(locmap_table,
  align = "|p{2cm}|p{3.5cm}|p{8cm}|") |> 
  print.xtable(floating = FALSE,
               hline.after = -1:nrow(locmap_table), 
               row.names = FALSE)
...
By default, the \code{24-2} pattern is used, consisting of 54 regularly spaced locations across the central visual field, but other grids are included (Table \ref{tab:locmaps}).
...
The returned object's list items \code{tdfun}, \code{pdfun}, and \code{glfun} are functions that compute the TD, PD, and global indices. And \code{tdpfun}, \code{pdpfun}, and \code{glpfun} obtain the corresponding probability levels. The function \code{ghfun} computes the GH from TD values. These functions are not to be called directly. Instead, use the following functions:
\begin{itemize}
...
This section describes how to use the visual field package for pointwise analysis of locations, calculation of global parameters that summarize the visual field, longitudinal analyses, and plotting of the results. 
...
To calculate the total deviation there is a dedicated function \code{gettd()} which calls the internal function from the \code{nv()} return object.
...
Again, there is a dedicated function \code{getpd()}:
...
Even healthy eyes may show unusually small total deviation values by chance. To interpret these values, we calculate the probability that a deviation reflects pathology using empirical distributions from the normative data \citep{IMF:Hei1987a, IMF:Heijl1991}. These are categorized into discrete probability levels:
...
Global indices summarize the visual field by applying summary statistics on sensitivity, total deviation or pattern deviation. These indices are shown in Table and calculated using \code{getgl()}. The resulting \code{data.frame} also contains the patient's age, eye, and date of the test.
...
The mean sensitivity (MS) is just the arithmetic mean of the sensitivity values of all fields except the blind spot:

\[\mathrm{MS} = \frac{1}{n}\sum_{i=1}^{n} S_i\]

For the HFA, the mean deviation (MD) is the weighted mean of the total deviation values, weighted by the standard deviation of the normal values for each location. This results in more weighting of central locations:

\[\mathrm{MD} = \frac{\sum_{i=1}^{n} \frac{1}{Var(S_{normal})} \cdot TD_i}{\sum_{i=1}^{n} \frac{1}{Var(S_{normal})}}\]

For characterizing local variation of the visual field, the HFA uses pattern standard deviaton (PSD), which is the weighted standard deviation of the PD values (i.e. the TD values minus the general height GH).

The visual field index (VFI) is a global parameter that, similar to the MD, summarizes the overall loss of sensitivity, but iss supposed to be less strongly affected by cataract \citep{IMF:Ben2008}. This is achieved by putting more weight on the specific locations. It is expressed in percent, where a VFI of 100\%corresponds to a normal field and 0\% corresponds to blindness. The Catarct (opacification of the crystalline lens) is an important confounder in longitudinal follow-up of glaucoma patients, because surgical treatments for glaucoma may trigger cataract and cataract leads to deterioration of the visual field in the absence of glaucoma progression. 
...
The \code{glr()} function estimates a regression model for global indices. For the example series, the regression can be calculated as follows:
...
We can generate normative values for both grids with data from normal subjects provided in the package (\code{vfctrIowaPC26} and \code{vfctrIowaPeri}) \citep{IMF:Marin2018c}:
...
We just set the locmap and the graphical parameters, and we created normative data from the normal visual fields using \code{nvgenerate()}. We do the same for the peripheral locations:
...
The tesselation simits should be identical.
...
Note that Octopus perimeters often use different standard settings. The current Octopus\,900 perimeter uses a different maximum luminance increment ($L_{\rm max} = 4000\,\rm asb$) \citep{CH:racette_visual_nodate}. Older Octopus perimeters frequently employed a background luminance of 4\,asb and a maximum luminance increment of $L_{\rm max} = 1000\,\rm asb$ \citep{CH:fankhauser_problems_1979}. Occasionally, these older settings are maintained on the Octopus\,900 to ensure comparability with prior examinations in patients with longitudinal follow-up examinations.

Furthermore, deviatons are reported as defects, where a positive value (defect, instead of a negative deviation) denotes loss of sensitivity, and MD is calculated as the arithmetic mean instead of the weighted mean.

Please refer to the guide "Visual Field Digest" provided by Haag-Streit, pp. 235-250, for a more comprehensive comparison \citep{CH:racette_visual_nodate}. 

\subsection{The Jansonius map of the retinal nerve fiber layer  and RGC displacement}
The \pkg{visualFields} package includes functions for the implementation of the Jansonius map \citep{IMF:Jan2012}, describing average never fiber bundle paths along the visual field from the optic nerve head (ONH). This model aids in comparisons between structural imaging and perimetry data in glaucoma. Functions are implemented to convert from Cartesian to the modified polar coordinates in the Jansonius map, the average bundle paths, and the angle of incidence of the average bundle passing through a specific visual field location on the ONH. One optical coherence tomgraphy (OCT) image modality consists of scanning along a circle centered at the center of the blind spot. A function returns the correspondence between angle of incidence of the average bundle on the ONH and the circular scan. These functions are:
...
For some analyses is important to understand the underlying structure of retinal elements that are stimulated during perimetry stimulus presentation. This is particularly important when the distance from the center of vision is less than 10 degrees of visual angle. The photoreceptors that are stimulated are connected to ganglion cells (whose axons travel in bundles described by the Jansonius map and form the optic never). For retinal location on this area, the photoreceptors are not at the same topographical location as the ganglion cell bodies \citep{IMF:Drasdo2007}. The displacement between photoreceptors and ganglion cell bodies follow the implementation described by Montesano et al. \citep{IMF:Montesano2020} using the function vf2gc, which takes in the the cartesian coordinates in visual degrees and provides the corresponding displaced coordinates in visual degrees. For the most common perimetry test targeting the central 10 degrees of the visual field, the 10-2 grid, the grid of stimulus locations and the corresponding ganglion cell bodies are shown in the next figure:
<<echo=TRUE, fig=TRUE>>=
