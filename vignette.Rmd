...
Usually, perimetry tests the ability to perceive differences in luminance with an achromatic (not colored) stimulus (white-on-white perimetry). However, theoretically, other aspects of vision can be tested at different locations (ability to discriminate dots that are very close to each other, ability to discriminate similar colors, contrast sensitivity). The visual fields package was created to analyze data from *static perimetry*, where a threshold is measured for each of a fixed number of locations in a grid pattern. In contrast, in *kinetic perimetry*, a light spot moves from the periphery toward the center along a number of meridians and the location there the spot is first perceivable for the subject is noted.
...
The spatial position of the points $l_1$ to $l_n$ in the visual field is found in the locmap and depends on the pattern used by the perimeter. When analysing data, you need to specify the locmap.
...
The visualField package contains some example data. This dataset contains the results of 42 visual field tests of one patient (id: sample1). The patient has a chronic-neurodegenerative disease of the optic nerve called primary open-angle glaucoma (type: pwg).
...
We use the dataset included in the package for demonstrating calculation of global parameters.
...
The standard already contains some additional information with a border presenting a color-coded visualization of the deviation and a graphical representation of the blind spot. Note that the vfplot with the option "td" needs information on differential light-sensitivities in a normal reference population and on the probability of given deviations from these normal values. These informations are included for the typical Humphrey Field Analyzer fields, but you need to collect this information if you use other parameters.
...
vfplot(example1, type = "tds")

```

```{r}

vfplot(example1, type = "pd")

```

```{r}

vfplot(example1, type = "pds")
...
The *glr* function performs linear regression on global parameters.
...
The *plr* function performs pointwise linear regression.
...
In contrast, a deterioration is very likely (p < 0.0001).
...
Octopus 900 Perimeter (Haag-Streit)

The loadoctopus function also imports some normative data from the Octopus perimeter by Haag-Streit, but this does not allow full use of the functionality of the visualFields package, currently.
...
## Locini and Number of locations / vfisvalid
